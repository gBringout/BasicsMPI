...
system.baseFrequency = 25e5; % [Hz] Frequency used to generate the one of the drive fields
system.freqDivider = [3*31 3*32 3*33]; % To generate the Lissajoue trajectorie
...
system.samplingFrequency = 17.5e6; %[Hz] Sampling Frequency of the acquisition Board.
%Maximal and minimal frequencies in the spectrum
system.fSMmin = 57000; %because of the filters
system.fSMmax = 1000000; % to keep the data usage low
...
%system.frequencyDrive2/calculation.deltaf
%1/(calculation.time(2)-calculation.time(1))
...
%(system.freq(2)-system.freq(1))/calculation.deltaf
...
system.x = -0.003:calculation.dx:0.003; %The resolution have to be limited in order to be able to reconstruct
system.y = -0.004:calculation.dy:0.004;
...
phantom.data = phantom.particleConcentration*createResolutionPhantomGael4(phantom.shape, 1);
%phantom.data = phantom.particleConcentration*createResolutionPhantomGael3(phantom.shape);
%figure;imagesc(system.x,system.y,phantom.data)
...
tic
c1 = cos(2*pi*system.frequencyDrive1*calculation.time(:)');
c2 = sin(2*pi*system.frequencyDrive2*calculation.time(:)');

c1_dt = cos(2*pi*system.frequencyDrive1*(calculation.time(:)'+calculation.dt));
...
clear('c1','c2','c1_dt','c2_dt')
fprintf('Time taken %2.0f s.\n', toc)
%% Display the fields
% figure
% threshold = 3*10^-3;
% for i=1:20:system.numberOfTimePoints
...
for i=2:size(results.signal2FFT_oneSided,2)
    if results.signal2SNR(i) > system.SNRLimits && system.freq(i) > system.fSMmin && system.freq(i) < system.fSMmax
...
subplot(4,1,3)
stem(system.freq/system.frequencyDrive1,abs(real(results.signal1FFT_oneSided)),'Marker','None')
%set(gca,'yscal','log')
xlabel('# Harmonic')
ylabel('Spectrum u1_1')
title('Spectrum u1_1')
xlim([0 30])
ylim([0 1])
...
xlim([0 30])
ylim([1 1000000000]);%std(signalSNR)])
% 
...
% 
% results.wantedFreq = [3 4 5 6]*system.frequencyDrive1;
% figure('Name','SM')
